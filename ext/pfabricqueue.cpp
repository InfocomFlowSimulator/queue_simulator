...
int comparePF(Packet* p1, Packet* p2) {
...
void PFabricQueue::enque(Packet *packet) {
    ...
    if (bytes_in_queue > limit_bytes) {
        ...
        for (uint32_t i = 0; i < packets.size(); i++) {
            if (comparePF(packets[i], packets[worst_index]) >=0)
            {
                worst_index = i;
            }
        }
...
Packet* PFabricQueue::deque() {
    if (bytes_in_queue > 0) {
        ...
        for (uint32_t i = 0; i < packets.size(); i++) {
            ...
            if (comparePF(curr_pkt, packets[best_index]) < 0)
            {
                best_index = i;
            }
        }
        ...
        for (uint32_t i = 0; i < packets.size(); i++) {
            Packet* curr_pkt = packets[i];
            if (curr_pkt->flow->id == packets[best_index]->flow->id) {
                best_index = i;
                break;
            }
        }
