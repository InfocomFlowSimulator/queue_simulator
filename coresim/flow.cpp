...
Flow::Flow(uint32_t id, double start_time, uint32_t size, Host *s, Host *d) {
    ...
    this->size_available = 0;
    this->pkt_drop_init = 0;


    this->min_work_time = 0;
}

void Flow::recalculateMinWorkTime(Topology* topology) {
    if (dataArrivalEvents.size() == 1)
    {
        min_work_time = topology->get_oracle_fct(this)*1e-6;
        return;
    }
    for (int j = 0; j < dataArrivalEvents.size(); j++)
    {
        uint32_t left = dataArrivalEvents.size()-j;
        //double leftTD = 1.0*src->queue->get_transmission_delay(params.mss)*(left-1);
        //double leftTDLast = 1.0*src->queue->get_transmission_delay(params.cut_through ? hdr_size : params.mss);
        uint64_t oldSize = this->size;
        this->size = left*params.mss;

        double candDiff = dataArrivalEvents[j]->time + topology->get_oracle_fct(this)*1e-6 - min_data_arrival_time;
        this->size = oldSize;
        min_work_time = std::max(min_work_time, candDiff);
    }
}
...
//            }
            ...
            if (received_ack_for.count(seq) == 0)
            {
                send(seq);
            }
...
Packet *Flow::send(uint32_t seq) {
    ...
    uint32_t hidden_priority = get_hidden_priority(seq);
    p = new Packet(
            get_current_time(), 
            this, 
            seq, 
    ...
    p->wa_srpt = size_available - last_unacked_seq - scoreboard_sack_bytes + 1;
    this->total_pkt_sent++;
...
void Flow::receive_ack(uint32_t ack, std::vector<uint32_t> sack_list) {
    ...
    for (uint32_t num : sack_list)
    {
        received_ack_for.insert(num);
    }
...
#define CORRECT_SRPT 10
#define WRONG_SRPT 20
#define LAZY_SRPT 30
...
uint32_t Flow::get_priority(uint32_t seq) {
    ...
    if(params.deadline && params.schedule_by_deadline)
    {
        ...
        if (params.srpt_mode == LAZY_SRPT)
        {
            return size_available == size ? get_hidden_priority(seq) : (uint32_t)(1e+9+1e+7);
        }
        return get_hidden_priority(seq);
    }
}

uint32_t Flow::get_hidden_priority(uint32_t seq) {
    if (params.flow_type == 1) {
        return 1;
    }
...
void Flow::additional_size_coming(size_t size)
{
    size_available += size;
    if (size_available == this->size && params.srpt_mode == LAZY_SRPT)
    {
        next_seq_no = last_unacked_seq;
    }
    send_pending_data();
